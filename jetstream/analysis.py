...
class Analysis:
    ...
    def _get_timelimits_if_ready(
        ...
        if (
            config.experiment.end_date != current_date
            or config.experiment.status != "Complete"
        ):
            return None

        analysis_length_dates = (
            (config.experiment.end_date - config.experiment.start_date).days
            - dates_enrollment
            + 1
        )
    ...
    def _table_name(self, window_period: str, window_index: int, normandy_slug) -> str:
    ...
    def _publish_view(self, window_period: AnalysisPeriod, normandy_slug, table_prefix=None):
        assert normandy_slug is not None
        normalized_slug = bq_normalize_name(normandy_slug)
    ...
    def check_runnable(self, config: AnalysisConfiguration, current_date: Optional[datetime] = None) -> bool:
    ...
    def run(self, current_date: datetime, config: AnalysisConfiguration, dry_run: bool = False) -> None:
        """
